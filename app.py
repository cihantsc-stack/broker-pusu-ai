...
def main():
    ...
    with top1:
        metric_box('CANLI SAAT', datetime.now().strftime('%H:%M:%S'), 'blue')
    ...
    with top2:
        metric_box('BIST100 Son Kapanış', t, k)

    t, k = idx_text('BIST30')
    with top3:
        metric_box('BIST30 Son Kapanış', t, k)
    ...
    if df_raw is None or df_raw.empty or len(df_raw) < 60:
        st.error('❌ Gerçek fiyat verisi alınamadı. Bu sürüm sahte/demo fiyat üretmez.')
        st.info(msg)
        st.stop()
